"""
ai_service.py — Groq AI agent service (mirrors services/aiService.js).
...
# ── Agent system prompts (identical to JS version) ───────────────────────────
AGENT_PROMPTS = {
...
_DEMO_RESPONSES = {
    ...
    "generalHealth": "Hello! I am Arogya Raksha AI, your personal healthcare assistant. How can I help you today?",
    "emergencyAgent": json.dumps({
        "emergencyLevel": "HIGH",
...
def get_demo_response(agent_type: str) -> str:
    return _DEMO_RESPONSES.get(agent_type, _DEMO_RESPONSES["generalHealth"])
...
async def call_gemini_agent(
    ...
    """
    Call Groq AI with the given agent type and message.
    Mirrors callGeminiAgent() from aiService.js.
    """
    system_prompt = AGENT_PROMPTS.get(agent_type, AGENT_PROMPTS["generalHealth"])
    try:
        completion = _client.chat.completions.create(
            model="llama-3.3-70b-versatile",
            messages=[
                {"role": "system", "content": system_prompt},
                {"role": "user", "content": f"Patient Context: {context}\n\nUser Message: {user_message}"},
            ],
            temperature=0.7,
            max_tokens=2000,
        )
        response_text = completion.choices[0].message.content or ""
        return {"success": True, "data": response_text, "agent": agent_type}
    except Exception as e:
        print(f"Groq AI error ({agent_type}): {e}")
        return {
            "success": False,
            "error": str(e),
            "agent": agent_type,
            "data": get_demo_response(agent_type),
        }
